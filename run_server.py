...
class DatabaseSyncHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif self.path == '/api/load_charging_methods':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            data_file = 'charging_methods.json'
            if os.path.exists(data_file):
                with open(data_file, 'r', encoding='utf-8') as f:
                    content = f.read()
            else:
    ...
    def do_POST(self):
        if self.path == '/api/save_audits':
            ...
            try:
                ...
                self.end_headers()
                self.wfile.write(json.dumps({"status": "error", "message": str(e)}).encode('utf-8'))
        elif self.path == '/api/save_charging_methods':
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length)
            try:
                data = json.loads(post_data.decode('utf-8'))
                with open('charging_methods.json', 'w', encoding='utf-8') as f:
                    json.dump(data, f, indent=2, ensure_ascii=False)
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({"status": "success", "message": "Charging methods saved"}).encode('utf-8'))
            except Exception as e:
                self.send_response(400)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
...
def open_browser():
    url = f"http://localhost:{PORT}/index_v1.html"
